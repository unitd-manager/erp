...
const Sidebar = () => {
  ...
  const storagePermit = localStorage.getItem('__permifyUser');
const { permissions } = JSON.parse(storagePermit) || { permissions: [] };
  ...
  menu.map((navi) => {
    if (navi.data) {
      let hasPermit = false;
      let count = 0;
      if (permissions) { // Check if permissions is defined
